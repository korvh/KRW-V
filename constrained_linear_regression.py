# original code: https://github.com/avidale/constrained-linear-regression
# adapted to enable min/max coef definition at model initialization, not model fit
...
class ConstrainedLinearRegression(LinearModel, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.min_coef is None:
            self.min_coef_ = np.repeat(-np.inf, X.shape[1])
        else:
            self.min_coef_ = self.min_coef

        if self.max_coef is None:
            self.max_coef_ = np.repeat(np.inf, X.shape[1])
        else:
            self.max_coef_ = self.max_coef
